...
<head>
	...
	<title>DEhuroad news agency</title>
...
<!-- 		<div class="container">
	...
	if(isset($_REQUEST['lname']) and $_REQUEST['lname']!=""){
		$condition	.=	' AND lname LIKE "%'.$_REQUEST['lname'].'%" ';
	}
	if(isset($_REQUEST['']) and $_REQUEST['lno']!=""){
		$condition	.=	' AND lno LIKE "%'.$_REQUEST['lno'].'%" ';
	}
	...
	/*if(isset($_REQUEST['df']) and $_REQUEST['df']!=""){
		...
		$condition	.=	' AND DATE(date(format))>="'.$_REQUEST['df'].'" ';

	}
	if(isset($_REQUEST['date']) and $_REQUEST['date']!=""){

		$condition	.=	' AND DATE(date)<="'.$_REQUEST['date'].'" ';

	}
	*/
	
	$userData	=	$db->getAllRecords('lane','*',$condition,'ORDER BY lno DESC');
	?>
   	<div class="container">
		<h1><a href="home.html">Dehuroad News paper agency</a></h1>
		<div class="card">
			<div class="card-header"><i class="fa fa-fw fa-globe"></i> <strong>Browse lane</strong> <a href="laneadd.php" class="float-right btn btn-dark btn-sm"><i class="fa fa-fw fa-plus-circle"></i> Add lane</a></div>
			<div class="card-body">
				<?php
				...
				?>
				<div class="col-sm-12">
					<h5 class="card-title"><i class="fa fa-fw fa-search"></i> Find lane</h5>
					<form method="get">
						<div class="row">
							<div class="col-sm-2">
								<div class="form-group">
									<label> Lane Number</label>
									<input type="text" name="lno" id="lno" class="form-control" value="<?php echo isset($_REQUEST['lno'])?$_REQUEST['lno']:''?>" placeholder="Enter lane no ">
								</div>
							</div>
							<div class="col-sm-2">
                                  <div class="form-group">
									<label>Lane Name</label>
									<input type="text" name="lname" id="lname" class="form-control" value="<?php echo isset($_REQUEST['lname'])?$_REQUEST['lname']:''?>" placeholder="Enter lane name">
								</div>
							</div>
							<!-- <div class="col-sm-2">
								<div class="form-group">
									<label>User Phone</label>
									<input type="tel" name="userphone" id="userphone" ="form-control" value="<?php //echo isset($_REQUEST['userphone'])?$_REQUEST['userphone']:''?>" placeholder="Enter user phone">
								</div>
							</div> -->
							<div class="col-sm-4">
		...
		<div>
			<table class="table table-striped table-bordered">
				...
				<tbody>
					...
					?>
					<tr>
						<td align="center"><?php echo $val['lno'];?></td>
						<td align="center"><?php echo $val['lname'];?></td>
						<!-- <td><?php //echo $val['userphone'];?></td> -->
						<!-- <td align="center"><?php //echo date('Y-m-d',strtotime($val['date']));?></td> -->
						<td align="center">
							<a href="laneupdate.php?editId=<?php echo $val['lno'];?>" class="text-primary"><i class="fa fa-fw fa-edit"></i> Edit</a> | 
							<a href="lanedelete.php?delId=<?php echo $val['lno'];?>" class="text-danger" onClick="return confirm('Are you sure to delete this user?');"><i class="fa fa-fw fa-trash"></i> Delete</a>
						</td>
